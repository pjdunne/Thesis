...
\end{equation}
where $\sigma\left(\PZ\rightarrow\nu\nu\right)$ is the cross-section for \Znunu and $\sigma\left(\PZ/\gamma^{*}\rightarrow\mu\mu\right)$ is the cross-section for \Zmumu. $\epsilon^{S}_{\mbox{\mathrm{VBF}}}$ and $\epsilon^{C}_{\mbox{\mathrm{VBF}}}$ are the efficiencies for $Z\rightarrow\nu\nu$ events to pass the signal region selection and $Z/\gamma^{*}\rightarrow\mu\mu$ events to pass the control region selection respectively. As Z bosons can be created via either \ac{QCD} (those where the vertex where the \PZ boson is created is the only electroweak vertex) or electroweak processes (those where there are multiple electroweak vertices), which both have different cross-sections and efficiencies, $\epsilon^{S}_{\mbox{\mathrm{VBF}}}$ and $\epsilon^{C}_{\mbox{\mathrm{VBF}}}$, are a cross-section weighted average of the efficiency for both types of production, calculated as:
\begin{align}
  \label{eq:zdataeffs}
...
\begin{table}
  \caption{The input variables for the calculation of $\epsilon^{S}_{\mbox{\mathrm{VBF}}}$ and $\epsilon^{C}_{\mbox{\mathrm{VBF}}}$ using Equations~\ref{eq:zdataeffs} and \ref{eq:zdataeffc} respectively.}
